...
real growq(real time, real q_final, real tau_grow) {
  real qt;
  if (time < tau_grow) {
    qt = pow(sin(M_PI/2 * time/tau_grow),2);
  } else {
    qt = 1;
  }
  return q_final*qt;
...
OMEGAFRAME (which is used afterwards to build the initial Vx field. */
  ...
  /* !!!!!!!!!!!!!!!!!!!!! NEW STUFF !!!!!!!!!!!!!!!!!!!!!!!!!!!!!! */
  // the mass of the planet is saved as the final mass the planet will reach
  real final_mass [Sys->nb];
  for (int np; np < Sys->nb; np++) {
    final_mass[np] = Sys->mass[np];
  }
  if (tau_grow > 0) {
    masterprint("************** GROWTH ***************\n");
    masterprint("Planet growth included\n");
    masterprint("The mass of the planet has been set to zero\n");
    masterprint("Growth timescale is %f = %f orbits\n",tau_grow,tau_grow/2/M_PI);
    masterprint("************* END GROWTH ************\n");
    for (int np; np < Sys->nb; np++) {
    Sys->mass[np] = 0;
  }
  }
  ...
  for (i = begin_i; i<=NTOT; i++) { // MAIN LOOP
    ...
    while (dtemp<DT) { // DT LOOP
	 ...
	 fluid */
      ...
      // at end of timestep update mass of planet
      for (int np=0; np < Sys->nb; np++){
        if (Sys->mass[np] < final_mass[np]) {
          Sys->mass[np] = growq(PhysicalTime, final_mass[np], tau_grow);
        }
      }
