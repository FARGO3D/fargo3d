...
void HallEffect_UpdateB(real dt,int idx, int idy, int idz) { 
  ...
  if(idx == 2) {                     //this flag allows Bx to be updated using 
    B = Bx;                          //the EmfzH.
    Emf1 = EmfyH;  
    Emf2 = EmfzH;
    idx1 = 0; idy1 = 1; idz1 = 0; 
  ...
  if(idz == 2) {                    //this allow update Bz only with
    B = Bz;                         //EmfyH as required Hall algorithm
    Emf1 = EmfxH;
    Emf2 = EmfyH;
    idx1 = 1; idy1 = 0; idz1 = 0; 
    idx2 = 0; idy2 = 1; idz2 = 0; 
  ...
  FARGO_SAFE(_HallEffect_UpdateB(dt, mutex, mutez, idx, idy, idz, idx1, idy1, idz1, idx2, idy2, idz2, B, Emf1, Emf2));

}

void _HallEffect_UpdateB_cpu(real dt, int mutex, int mutez,int idx,int idy,int idz,int idx1,int idy1,int idz1, int idx2,int idy2,int idz2,Field* B,Field* Emf1,Field* Emf2) {
...
//<#>
	...
	b[l] += dt/surf * (  (emf1[lp2]*dp2p - emf1[l]*dp2m)*mutez
			   - (emf2[lp1]*dp1p - emf2[l]*dp1m)*mutex);
